...
def interactive_mode():
    ...
    print("  select <module_id>      - Select a module")
    ...
    while True:
        try:
            ...
            if command[0] == "quit":
                ...
                module_id = int(command[1])
                iomod.select_module(module_id)
            elif command[0] == "dw" and len(command) == 4:
                module_id, pin, value = int(command[1]), int(command[2]), int(command[3])
                iomod.digitalwrite(module_id, pin, value)
            elif command[0] == "dr" and len(command) == 3:
                module_id, pin = int(command[1]), int(command[2])
                ...
                module_id, pin, mode = int(command[1]), int(command[2]), int(command[3])
                iomod.pinmode(module_id, pin, mode)
            elif command[0] == "vref" and len(command) == 3:
                module_id, activate = int(command[1]), command[2].lower() == "on"
